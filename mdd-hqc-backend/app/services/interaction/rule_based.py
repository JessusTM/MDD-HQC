from app.models.llm_contract import InteractionInput, InteractionReport, InteractionQuestion, InteractionProposal 
...
class RuleBasedInteractionEngine(InteractionEngine):
    def run(self, payload: InteractionInput) -> InteractionReport:
        ...
        for node in payload.nodes:
            if node.get("type") == "goal" and not node.get("text"):
        ...
        for link in payload.links:
            if link.get("type") == "refinement":
                proposals.append(InteractionProposal( 
                    id="p_refinement", 
                    kind="add_or_group_child", 
                    target={"parent": link.get("source")}, 
                    data={"child": link.get("target")}, 
